...
| 路径 | 说明 |
|------|------|
| `.agents/skills/` | Skill 定义（每个 skill 一个目录，含 SKILL.md） |
| `.claude/` | Claude Code CLI 配置 |
| `.gemini/` | Gemini CLI 配置（settings.json + commands/*.toml） |
| `.codex/` | Codex AI 配置 |
...
4. 在 `.gemini/commands/` 添加对应 `.toml` 配置
5. 在 `CLAUDE.md` 和 `AGENTS.md` 的 Skills 列表中注册
